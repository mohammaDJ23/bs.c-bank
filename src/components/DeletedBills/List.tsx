...
import { DeletedBillListFilters, getTime, isoDate, listScrollTop } from '../../lib';
  ...
  return (
    <>
          ...
          <MuiList>
            {deletedBillsList.list.map((bill, index) => (
              <ResetStyleWithAnimation key={index} sx={{ opacity: '1', transform: 'translateY(0)' }}>
                ...
                >
                  <BillCard index={index} bill={bill} list={deletedBillsList} />
                </Box>
      ...
      <Filter name={ModalNames.DELETED_BILL_FILTERS}>
        ...
        >
          <TextField
            id="_bank-service-deleted-bill-filters-form-search"
            label="Search"
            variant="standard"
            type="text"
            fullWidth
